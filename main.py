#!/usr/bin/env python3
"""Unified entry point for Med-MMFL: A Multimodal Federated Learning Benchmark in Healthcare.

Runs federated learning experiments across heterogeneous multimodal
medical datasets using configurable aggregation algorithms.

Usage::

    # BraTS24 (3-D segmentation) with FedAvg
    python main.py --config configs/fedavg.yml --algorithm fedavg \\
                   --comm-rounds 30 --name brats_fedavg

    # EHRXQA (generative VQA) with SCAFFOLD
    python main.py --config configs/ehrxqa_scaffold.yml --algorithm scaffold \\
                   --comm-rounds 50 --name ehrxqa_scaffold

    # PathVQA (yes/no VQA) with CreamFL
    python main.py --config configs/pathvqa_creamfl.yml --algorithm creamfl \\
                   --comm-rounds 40 --name pathvqa_creamfl

    # Enable Weights & Biases logging
    python main.py --config configs/fedavg.yml --algorithm fedavg \\
                   --wandb --wandb-project my_project

The script reads the ``dataset.dset_name`` field from the YAML config
to resolve the correct dataset-specific server, then runs the standard
federated loop: ``dispatch → update → evaluate``.

See Also:
    - ``med-mmfl-bench federated --help`` (installed CLI equivalent)
...
logger = get_logger(__name__, log_file="logs.txt")
...
def _resolve_server_class(
    ...
    try:
        ...
        logger.error(
            "Server module '%s.%s' not found. Check that the "
            "algorithm '%s' is implemented for dataset '%s'.",
            server_pkg,
            algorithm,
            algorithm,
            dset_name,
...
def main() -> None:
    ...
    logger.info("Dataset: %s | Algorithm: %s", config.dataset.dset_name, args.algorithm)
