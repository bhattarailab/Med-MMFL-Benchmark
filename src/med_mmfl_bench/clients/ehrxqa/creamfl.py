"""CreamFL client for EHRXQA federated VQA.

Implements client-side CREAM (Contrastive Representation Aggregation
for Multimodal FL) with intra-modal contrastive learning between
the current, global, and previous model representations.
"""
...
class CreamflClient(FedavgClient):
    """CreamFL client for EHRXQA.

    After standard local training, performs CREAM distillation using
    intra-modal contrastive loss to align local representations with
    global features.
    ...
    def extract_features(
        ...
        Args:
            model: Model with ``return_repr`` toggle.
    ...
    def compute_contrastive_loss_intra(
        ...
        Positive pair: student vs global (target).
        Negative pair: student vs previous local (old).

        Args:
            student_feat: Current model features ``(B, D)``.
            target_feat: Global model features ``(B, D)``.
            old_feat: Previous local model features ``(B, D)``.
    ...
    # ------------------------------------------------------------------
    # CREAM distillation step
    # ------------------------------------------------------------------

    def cream_loss_update(self) -> None:
        """Compute CREAM loss with intra-modal contrastive learning.

        Updates the model batch-by-batch using the public eval set.
        """
    ...
    def update(
        ...
        Args:
            global_text_feat: Global text representations from the server.
            global_vision_feat: Global vision representations from the server.
        ...
        # Save old model for contrastive distillation
