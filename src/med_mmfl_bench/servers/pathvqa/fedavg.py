"""FedAvg server for PathVQA (Yes/No visual question answering).

Orchestrates federated learning across clients using the FedAvg
aggregation strategy for classification-based VQA. Handles model
initialization, client management, evaluation with accuracy metrics,
...
class FedavgServer(BaseServer):
    """FedAvg server for PathVQA federated training.

    Manages the global classification model, dispatches it to clients,
    ...
    def _get_algorithm(self, **kwargs: Any) -> Any:
        """Dynamically load the FL aggregation optimizer.
    ...
    def _create_clients(
        ...
        client_module = import_module(
            f"med_mmfl_bench.clients.pathvqa.{self.args.algorithm}", package=__package__
        )
    ...
    def validate_model(self) -> float:
        ...
        Returns:
            The primary validation metric (accuracy_average).
        """
        ...
        with torch.no_grad():
            for batch in tqdm(self.val_dataloader, unit="batch", desc="Val"):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.float16):
                    output = self.global_model(**batch)["logits"]
        ...
        self.val_track.append(metrics["accuracy_average"])
        self.evaluator.reset()
        self.global_model.to("cpu")
        return metrics["accuracy_average"]
    ...
    def test(self) -> None:
        ...
        with torch.no_grad():
            for batch in tqdm(self.test_dataloader, unit="batch", desc="Test"):
                ...
                with torch.autocast(device_type="cuda", dtype=torch.float16):
                    outputs = self.global_model(**batch)["logits"]

                self.evaluator.update(outputs, label)
