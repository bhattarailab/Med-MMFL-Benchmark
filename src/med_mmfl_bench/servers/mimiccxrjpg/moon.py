"""MOON server for MIMIC-CXR multimodal classification.

Implements Model-Contrastive Federated Learning (MOON), which adds a
contrastive loss between the current local model, the global model, and the
previous local model to improve local training.
"""
...
class MimicMoonTrainer(MimicClientTrainer):
    """MIMIC-CXR trainer with MOON contrastive loss."""
    ...
    def moon_loss(
        ...
        Contrasts the current model's features against the global model
        (positive) and the previous local model (negative).
        """
    ...
    def run(self, comms: int) -> None:
        """Run local training and update the previous model snapshot."""
        super().run(comms)
        self.prev_net.load_state_dict(self.model.state_dict())


class MoonServer(FedavgServer):
    """MOON server — inherits FedAvg and swaps in a contrastive trainer."""

    def __init__(self, args: Any, config: Any, wandb: Any = False) -> None:
        super().__init__(args, config, wandb)
        self.trainer = MimicMoonTrainer(self.args, self.config, self.wandb)
