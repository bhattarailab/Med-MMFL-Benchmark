"""CreamFL server for SymileMIMIC multimodal retrieval.

This is a standalone server (not inheriting FedavgServer) because CreamFL
uses a fundamentally different protocol with representation distillation.
"""
...
class CreamflServer:
    """CreamFL server for SymileMIMIC with contrastive representation distillation.

    Standalone server class (separate from FedavgServer) because CreamFL
    requires public-data distillation at the server level.
    ...
    Args:
        ...
        server_trainset: Optional server-side public training dataset.
    ...
    def __init__(
        ...
        if self.train_dataset is None:
            ...
            last_client = self.clients.pop()
            del(last_client)
            import gc; gc.collect()

        self.val_loader, self.test_loader = self._get_dataloader()

        eval_size = int(0.1 * len(self.train_dataset))
        self.eval_loader = self._create_dataloader(
            Subset(self.train_dataset, range(eval_size)),
            shuffle=False
        )
    ...
    def set_trainer(self) -> None:
        """Configure the server-side trainer."""
    ...
    def _get_algorithm(self, **kwargs: Any) -> Any:
        ...
        self.server_optimizer = algo_module.__dict__[cls_name](
            model=self.trainer.model, **kwargs
        )
    ...
    def update(self) -> None:
        ...
        1. Train global model on public holdout data.
        ...
        global_cxr_feat, global_ecg_feat, global_labs_feat, _ = self.trainer.eval(
            self.eval_loader
        )

        # Client updates
        for client in self.clients:
            client.update(global_cxr_feat, global_ecg_feat, global_labs_feat, self.eval_loader)
    ...
    def evaluate(self) -> None:
        """Evaluate the best global model (placeholder)."""
        return None
