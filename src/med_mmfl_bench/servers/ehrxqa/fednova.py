"""FedNova server for EHRXQA federated VQA.
...
class FednovaServer(FedavgServer):
    ...
    def _aggregate(self) -> Dict[str, torch.Tensor]:
        ...
        Collects per-client normalisation coefficients ``a_i`` and
        gradient directions ``d_i``, then delegates to the server
        optimizer.
    ...
    def update(self) -> None:
        ...
        Uses normalised gradient averaging instead of direct weight
        averaging.
        """
        self.dispatch()
