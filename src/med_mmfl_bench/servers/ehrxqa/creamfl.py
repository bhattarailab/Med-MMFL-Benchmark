"""CreamFL server for EHRXQA federated VQA.

CreamFL (Contrastive Representation Aggregation for Multimodal
Federated Learning) uses contrastive-weighted feature aggregation
and knowledge distillation to improve the global model beyond
...
class CreamflServer(FedavgServer):
    """CreamFL server for EHRXQA.

    Extends FedAvg with:
        1. Optional server-side training on a public dataset.
        2. Global feature extraction from the server model.
        3. Contrastive-weighted aggregation of client features.
        4. Knowledge distillation of aggregated features back into
           the global model.
    ...
    Args:
        ...
        server_trainset: Optional public training dataset. If None,
            the last client's dataset is repurposed.
    ...
    def __init__(
        ...
        # Use part of training data as public dataset for feature extraction
        ...
        # Create eval subset for representation extraction (10% of training data)
    ...
    def extract_global_features(self) -> tuple:
        """Extract global text and vision representations from the server model.
    ...
    def aggregation(self) -> Dict[str, torch.Tensor]:
        ...
        Each modality is weighted based on its alignment with the
        global features. Returns aggregated text and vision features.
        """
        device = self.local_text_feat[0].device
        num_clients = len(self.local_text_feat)

        text_feats = torch.stack(self.local_text_feat).to(device)     # (C, N, D)
        vision_feats = torch.stack(self.local_vision_feat).to(device)  # (C, N, D)

        def calc_weighted_features(
            main_feats: torch.Tensor, ref_feats: torch.Tensor
        ) -> torch.Tensor:
            """Compute contrastive-weighted aggregation.

            Args:
                main_feats: Client features ``(n_clients, N, D)``.
                ref_feats: Reference (global) features ``(N, D)``.
    ...
    def update(self) -> None:
        ...
        Steps:
            1. (Optional) Train global model on server data.
            2. Extract global features.
            3. Dispatch to clients and perform local training + CREAM.
            4. Collect local features.
            5. Aggregate features with contrastive weighting.
            6. Distill into global model.
            7. Evaluate.
        """
        logger.info("=" * 50)
        ...
        # Steps 3 & 4: Update clients and collect local features
